...
int main()
{
   ...
   if(fd == -1) {
    ...
    fd_m55_hp_mhu0_ept = open(RPMSG_ENDPOINT, O_RDWR);
    if (fd_m55_hp_mhu0_ept == -1) {
       printf("Unable to open %s file .. please try again \n", RPMSG_ENDPOINT);
       printf("If %s file not found, create using mknod %s c 253 1 \n",
