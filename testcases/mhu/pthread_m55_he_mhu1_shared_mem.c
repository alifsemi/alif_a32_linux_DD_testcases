 ...
 *
 * MHU Doorbell + Shared SRAM1 corner-case test: A32 <-> M55-HE  (MHU1)
 *
 ...
 *
 * M55-HE echoes each byte XOR'd with (iteration + 0x42); A32 verifies.
 */
...
/* ---- Shared memory constants (must match Zephyr HE MHU1 side) ---- */
#define SHARED_MEM_BASE    0x027DDE00
#define A32_TO_HE_OFFSET   0x0000
#define HE_TO_A32_OFFSET   0x0100
#define SHARED_MEM_SIZE    0x0200

#define MAGIC_A32          0xA32F055E
#define MAGIC_HE           0xF055EA32
...
#define RX_TIMEOUT_MS  10000  /* 10s timeout waiting for HE response */
...
/* HE MHU1 endpoints */
#define EPT_TX_NAME       "txdb3"
#define EPT_RX_NAME       "rxdb3"
...
int main(void)
{
	...
	printf("==========================================\n");
	printf("A32 <-> M55-HE : MHU1 Corner-Case Test\n");
	printf("  SRAM base:         0x%08x\n", SHARED_MEM_BASE);
	printf("  A32->HE region:    0x%08x\n", SHARED_MEM_BASE + A32_TO_HE_OFFSET);
	printf("  HE->A32 region:    0x%08x\n", SHARED_MEM_BASE + HE_TO_A32_OFFSET);
	...
	tx_region = (volatile struct shared_msg *)
			((char *)sram_base + offset_in_page + A32_TO_HE_OFFSET);
	rx_region = (volatile struct shared_msg *)
			((char *)sram_base + offset_in_page + HE_TO_A32_OFFSET);
	...
	for (iter = 0; iter < ITERATIONS && !stop_flag; iter++) {
		...
		doorbell_tx = SHARED_MEM_BASE + A32_TO_HE_OFFSET;
		...
		printf("A32: Waiting for HE response...\n");

		if (iter > 0) {
			struct pollfd pfd = { .fd = fd_rx, .events = POLLIN };
			int pret = poll(&pfd, 1, RX_TIMEOUT_MS);
			if (pret == 0) {
				printf("ERROR: Timeout waiting for HE response "
				       "(%d ms)\n", RX_TIMEOUT_MS);
				fail_count++;
				break;
			}
		...
		if (doorbell_rx != (SHARED_MEM_BASE + HE_TO_A32_OFFSET)) {
