...
#define RPMSG_ENDPOINT "/dev/rpmsg2"


struct rpmsg_endpoint_info m55_he_mhu0_eptinfo = {"m55_he_mhu0", 0XFFFFFFFF, 0xFFFFFFFF};
pthread_mutex_t mutex1 = PTHREAD_MUTEX_INITIALIZER;
int fd, fd_m55_he_mhu0_ept;
...
{
   int status;
   status = ioctl(fd_m55_he_mhu0_ept, RPMSG_DESTROY_EPT_IOCTL);
   if (status == -1) {
       printf("Unable to destroy fd_m55_he_mhu0_ept endpoint correctly \n");
   }
   close(fd_m55_he_mhu0_ept);
   close(fd);
...
int main()
{
   ...
   printf("MHU0 TEST BETWEEN A32 and M55_HE cores \n");
   printf("===================================== \n");
   ...
   if(fd == -1) {
     ...
     status = ioctl(fd, RPMSG_CREATE_EPT_IOCTL, &m55_he_mhu0_eptinfo);
     if (status == -1) {
    ...
    fd_m55_he_mhu0_ept = open(RPMSG_ENDPOINT, O_RDWR);
    if (fd_m55_he_mhu0_ept == -1) {
       printf("Unable to open %s file .. please try again \n", RPMSG_ENDPOINT);
       printf("If %s file not found, create using mknod %s c 253 3 \n",
...
void * print_message_function_send( void *ptr )
{
	...
	pthread_mutex_lock( &mutex1 );
        status = write(fd_m55_he_mhu0_ept, &data, sizeof(data));
	if(status == -1) {
...
void * print_message_function_receive( void *ptr )
{
	...
	status = read(fd_m55_he_mhu0_ept, &data, sizeof(data));
