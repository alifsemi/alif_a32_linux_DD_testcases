 ...
 *
 * MHU Doorbell + Shared SRAM1 corner-case test: A32 <-> M55-HP  (MHU0)
 *
...
/* ---- Shared memory constants (must match Zephyr side) ---- */
...
/* Per-iteration test descriptor */
struct test_vector {
	const char *name;
	uint32_t    len;           /* payload length for this iteration */
	uint8_t     pattern[MAX_PAYLOAD];  /* data (only first 'len' bytes used) */
};

/* Build the 25 test vectors (15 normal + 10 corner-case) */
...
static void init_test_vectors(void)
{
	...
	/* [10] Single byte - 1 B (minimum payload) */
...
/* HP MHU0 endpoints */
#define EPT_TX_NAME       "txdb0"
#define EPT_RX_NAME       "rxdb0"
 ...
 * @ept_name: Endpoint name to match (e.g. "txdb0")
...
int main(void)
{
	...
	printf("==========================================\n");
	printf("A32 <-> M55-HP : MHU0 Corner-Case Test\n");
